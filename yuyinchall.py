...
def BiRNN_model( batch_x, seq_length, n_input, n_context,n_character ,keep_dropout):
    ...
    #第二个n_steps为时序数，第三个是Mfcc_features,取当前特征n_input和上下文特征数的组合，每个序列特征数为n_input
    #此时batch_x的大小为（？，？，494），此处的494 = 26+2*26*9
    batch_x_shape = tf.shape(batch_x)
    print('----------------batch_x_shape.shape:',batch_x_shape.shape)
    
    # 将输入转成时间序列优先 将batch_size调到第一维，将原来的时序长度调到第0维
    batch_x = tf.transpose(batch_x, [1, 0, 2])
    print('----------------batch_x.shape:',batch_x.shape)
    # 再转成2维传入第一层
    batch_x = tf.reshape(batch_x,
                         [-1, n_input + 2 * n_input * n_context])  # (n_steps*batch_size, n_input + 2*n_input*n_context)此处变成了两维
    print('----------------batch_x.shape:',batch_x.shape) #（？，494）
    ...
    with tf.name_scope('fc1'):
        ...
        print('----------------layer_1.shape:',layer_1.shape) #（？，1024）
    ...
    with tf.name_scope('fc2'):
        ...
        print('----------------layer_2.shape:',layer_2.shape) #（?,1024）
    ...
    with tf.name_scope('fc3'):
        ...
        print('----------------layer_3.shape:',layer_3.shape) #（?,2048）
    # 双向rnn
    with tf.name_scope('lstm'):
        ...
        print('----------------layer_3_lstm.shape:',layer_3.shape) #（?,2048)
        ...
        print('----------------outputs_states.shape:',outputs_states.shape) #（?,）
        sys.exit(0)
...
wav_path='/mnt/d/ASR/birnn-asr/data/wav/train'
label_file='/mnt/d/ASR/birnn-asr/data/doc/trans/train.word.txt'
